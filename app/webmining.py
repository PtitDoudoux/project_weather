...
class MeteoFrance(object):
    ...
    def __init__(self, city, postal_code):
        """
        | Initialise the MeteoFrance object with a given city and postal code
        |
        | :raises MeteoFranceError: Raise an exception if no city or postal code is inquired
        """

        if city is None and postal_code is not None:
            self.postal_code = str(postal_code)
            self.city = helpers.get_city(str(postal_code)).lower()
        elif city is not None and postal_code is None:
            self.city = city.lower()
            self.postal_code = helpers.get_postal_code(city)
        elif city is not None and postal_code is not None:
            self.postal_code = str(postal_code)
            self.city = city.lower()
        else:
            raise MeteoFranceError("You should at least inquire either the postal code or the city")

        self.session = requests.Session()
        self.weather = self.__retrieve_web_page_content()
    ...
    def retrieve_daily_weather(self, timestamps):
        ...
        weather_infos = {
            ...
            "weather_icon": helpers.get_weather_icon(weather_datas[4]),
            "uv": weather_datas[5],
            "wind": ' '.join(weather_datas[6:]) # TODO: Add wind direction
        }
    ...
    def retrieve_next_week_weather(self):
        ...
        for i in range(8, 14):
            ...
            formatted_wd = {
                ...
                "weather_type": weather_datas[4],
                "weather_icon": helpers.get_weather_icon(weather_datas[4]),
                "wind": ' '.join(weather_datas[6:9]) if len(weather_datas) > 9 \
                    else weather_datas[6],
    ...
    def __retrieve_web_page_content(self):
        """
        ...
        if helpers.is_empty(weather) is True:
